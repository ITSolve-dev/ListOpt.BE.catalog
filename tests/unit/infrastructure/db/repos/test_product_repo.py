...
class TestProductRepo:
    ...
    @pytest.fixture
    def product_repo(self, session_mock: AsyncMock) -> ProductRepoSQL:
    ...
    async def test_get(
        ...
        product_repo: ProductRepoSQL,
        session_mock: AsyncMock,
        id: int,
    ):
        mocker.patch(
            "catalog.infrastructure.db.repos.product_repo.selectinload",
        )
        filter_by_mock = mocker.MagicMock()
        select_stmt = mocker.MagicMock()
        select_stmt.filter_by = filter_by_mock
        product_repo._base_select_stmt = mocker.MagicMock(
            return_value=select_stmt
        )
        await product_repo.get(id)
        product_repo._base_select_stmt.assert_called_once()
    ...
    async def test_get_invalid_id(
        self, product_repo: ProductRepoSQL, session_mock: AsyncMock, id: int
    ):
