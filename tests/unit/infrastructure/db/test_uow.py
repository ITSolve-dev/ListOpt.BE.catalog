...
from catalog.infrastructure.db.repos import (
    CartRepoSQL,
    CategoryRepoSQL,
    ProductRepoSQL,
)
...
class TestSQLAlchemyUnitOfWork:
    ...
    async def test_handle_context_manager(self, mocker: MockerFixture):
        db_connection_mock = mocker.AsyncMock()
        uow = SqlAlchemyUnitOfWork(db_connection_mock)
        cart_repo_init = mocker.patch.object(
            CartRepoSQL, "__init__", return_value=None
        )
        ...
        async with uow:
            ...
            category_repo_init.assert_called_once_with(
                db_connection_mock.session
            )
            product_repo_init.assert_called_once_with(
                db_connection_mock.session
            )
