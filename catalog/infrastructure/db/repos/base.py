...
class BaseRepoSQL[EntityT: Entity]:
    ...
    async def _get(self, id: int) -> Optional[EntityT]:
    ...
    def _pagination_stmt(self, page: int, page_size: int) -> Select[tuple[EntityT]]:
    ...
    async def get_by_ids(self, ids: list[int]) -> Sequence[EntityT]:
        stmt = self._base_select_stmt().where(cast_inst_attr(self._entity.id).in_(ids))
